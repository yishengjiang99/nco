...
int main()
{
	...
	for (int i = 0; i < NUM_OSCILLATORS; i++)
	{
		...
		oscillator[i].phaseIncrement = 0; // WAVETABLE_SIZE * frequency / 48000 / 2 / PI;
...
#define bit32_normalization 4294967296.0f
	...
	FILE *fp = popen("ffplay -ac 1 -ar 48k -f u32le -i pipe:0", "w");

#define midi_to_hz(midiPitch) 440.0f * powf(2.0f, (float)(midiPitch - 69) / 12.0f);

#define phaseIncrement(frequency) (int32_t)(bit32_normalization * frequency / sampleRate + 0.5f)

#define play_midi(midi, second)                                    \
	oscillator[0].phaseIncrement = phaseIncrement(midi_to_hz(midi)); \
	for (int l = 0; l < sampleRate * second; l += SAMPLE_BLOCKSIZE)  \
	{                                                                \
		wavetable_1dimensional_oscillator(&oscillator[0]);             \
		bzero(oscillator[0].output_ptr, 4 * SAMPLE_BLOCKSIZE);         \
		fwrite(oscillator[0].output_ptr, SAMPLE_BLOCKSIZE, 4, fp);     \
	}

	float renderTime = 0;
	float lookAheadBuffer = 129 / sampleRate;

	tml_message *m = tml_load_filename("song.mid");

	int note = 44;
	play_midi(note, 0.5);
	play_midi(note += 2, 0.5); // + incup, 0.5);
	play_midi(note += 2, 0.5); // + incup, 0.5);
	play_midi(note += 2, 0.5); // + incup, 0.5);
	play_midi(note += 2, 0.5); // + incup, 0.5);
	play_midi(note += 2, 0.5); // + incup, 0.5);
