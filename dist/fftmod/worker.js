...
function init(wasmModule) {
    ...
    const dataArray = wasmModule.HEAPF64.subarray(startAddr, startAddr + ringsize);
    const stateArray = wasmModule.HEAPU32.subarray(statePtr << 2, statePtr << (2 + 2 * Uint32Array.BYTES_PER_ELEMENT));
    const ringbuf = {
        get wptr() {
