...
function osc_info(ref) {
    ...
    const [wv00, wv01, wv10, wv11] = new Uint32Array(Module.mem.buffer, osc_ref + 15 * Float32Array.BYTES_PER_ELEMENT, 6);
    return {
        output_ptr,
        samples_per_block,
        phase,
...
function onMSG(e) {
    const { data, target } = e;
    const { readable, setMidiNote, setFade, setFadeDelta, setPhaseIncrement, keyOn, keyOff, info, setTable, } = e.data;
    if (readable) {
    ...
    if (setTable) {
        ...
        tbViews[channel].setUint32(tbIndex * Uint32Array.BYTES_PER_ELEMENT, waveTableRegistry[formIndex], true);
