export default function io_samplers(ctx, fftSize, post_data) {
    ...
    var input_freq = line_chart("#canvasA");
    var output_freq = line_chart("#canvasB");
    var sample_timer;
    ...
    function run_samples() {
        function sample() {
            ...
            var sum = 0;
            inputAnalyzer.getFloatTimeDomainData(dataArrayIn);
            dataArrayIn.map((d) => (sum += d * d));
            var inputrms = Math.floor(Math.sqrt(sum));
            sum = 0;
            outputAnalyzer.getFloatTimeDomainData(dataArrayOut);
            dataArrayOut.map((d) => (sum += d * d));
            var outputrms = Math.floor(Math.sqrt(sum));
            var dataArrayIn2 = new Uint8Array(fftSize);
            var dataArrayOut2 = new Uint8Array(fftSize);
