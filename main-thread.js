...
let statebuffer;
let midiStream;
let midiWriter; const d1 = document.querySelector("div");

const main = document.querySelector("pre"); const canvas = document.querySelector("canvas");
...
export async function init_audio_ctx() {
    ...
    function load_preset(pid, bankId, cid) {
        fetch(`pages/${pid}_${bankId}.dat`).then(res => res.arrayBuffer()).then(ab => {
            awn.port.postMessage({preset: ab, channel: cid}, [ab]);
        });
    }
    awn.port.postMessage({midi_readable: midistream.readable}, [midistream.readable]);

    load_preset(0, 0, 0); load_preset(10, 0, 0);
    ...
    awn.connect(ctx.destination); const w = 10, h = 10;
    ...
    let uivel = 0x7f;
    window.onkeydown = (e) => e.repeat === false && k2m(e) > -1 && awn.port.postMessage({noteOn: [uichannel, k2m(e), uivel]});
    window.onkeyup = (e) => k2m(e) > -1 && awn.port.postMessage({noteOn: [uichannel, k2m(e), uivel]});
    function loop() {
        if (statebuffer) {
