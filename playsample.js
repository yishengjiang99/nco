...
let fftmodw;
let fftInputBuffer = new Float32Array(256);
let fftspectraRef;
fftmod(12).then((mod) => {
...
canvasContainer.style =
  "position:fixed;right:0;top:0px;display:grid;align-items:right";
...
async function processClicks(e) {
  ...
  if (e.target.classList.contains("attlist")) {
    ...
    document.querySelector("#details").innerHTML = attrs
      .map((attr, index) => `${genstrs[index]}:${attr}`)
      .join("<br>");
    return;
  }
